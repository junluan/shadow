...
inline float deform_im2col_bilinear(const float* in_data, int in_h, int in_w,
                                    float h_im, float w_im) {
  auto h_low = static_cast<int>(std::floor(h_im));
  auto w_low = static_cast<int>(std::floor(w_im));
  ...
  for (int c_im = 0; c_im < in_c; ++c_im) {
    for (int h_col = 0; h_col < out_h; ++h_col) {
      for (int w_col = 0; w_col < out_w; ++w_col) {
        ...
        const auto* offset_data_ptr = offset_data + offset_data_offset +
                                      deform_group_idx * 2 * kernel_size_h *
                                          kernel_size_w * out_h * out_w;
        ...
        for (int i = 0; i < kernel_size_h; ++i) {
          for (int j = 0; j < kernel_size_w; ++j) {
            ...
            auto offset_h =
                offset_data_ptr[(offset_data_idx * out_h + h_col) * out_w +
                                w_col];
            auto offset_w =
                offset_data_ptr[((offset_data_idx + 1) * out_h + h_col) *
                                    out_w +
                                w_col];
