...
void DeformIm2Col2D<DeviceType::kCPU, float>(
  ...
  for (int c = 0; c < in_c; ++c, in_data += in_h * in_w) {
    ...
    int om_offset =
        deform_group_idx * kernel_size_h * kernel_size_w * out_spatial;
    const auto* offset_data_ptr = offset_data + 2 * om_offset;
    const auto* mask_data_ptr = use_mask ? mask_data + om_offset : nullptr;
    for (int k_s = 0; k_s < kernel_size_h * kernel_size_w;
         ++k_s, offset_data_ptr += out_spatial) {
      ...
      for (int h = 0; h < out_h; ++h, h_offset += stride_h) {
        ...
        for (int w = 0; w < out_w;
          ...
          auto w_in = w_offset + offset_data_ptr[out_spatial];
          auto mask_val = use_mask ? *mask_data_ptr++ : 1.f;
          if (h_in > -1 && h_in < in_h && w_in > -1 && w_in < in_w) {
            *col_data++ = mask_val * deform_im2col_bilinear(in_data, w_in, h_in,
                                                            in_w, in_h);
          } else {
            *col_data++ = 0.f;
          }
