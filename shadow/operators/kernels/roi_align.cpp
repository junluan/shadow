...
inline float roi_align_bilinear(const float* data, float x, float y, int width,
                                int height) {
...
template <>
void ROIAlign<DeviceType::kCPU, float>(
  ...
  for (int n = 0; n < num_rois; ++n) {
    ...
    float roi_start_w = roi_data[roi_offset + 1] * spatial_scale - offset;
    float roi_start_h = roi_data[roi_offset + 2] * spatial_scale - offset;
    float roi_end_w = roi_data[roi_offset + 3] * spatial_scale - offset;
    float roi_end_h = roi_data[roi_offset + 4] * spatial_scale - offset;
        ...
        for (int pw = 0; pw < pooled_w; ++pw) {
          ...
          for (int h = 0; h < roi_bin_grid_h; ++h) {
            ...
            for (int w = 0; w < roi_bin_grid_w; ++w) {
              ...
              sum_val += roi_align_bilinear(in_data_ptr, x, y, in_w, in_h);
            }
          }
