...
inline int deconv_out_size(int dim, int kernel_size, int stride, int pad,
                           int dilation) {
  return stride * (dim - 1) - 2 * pad + dilation * (kernel_size - 1) + 1;
}
...
class DeconvOp : public Operator {
 public:
  DeconvOp(const shadow::OpParam& op_param, Workspace* ws)
    ...
    kernel_ = std::dynamic_pointer_cast<DeconvKernel>(
  ...
  void Run(const std::vector<std::shared_ptr<Blob>>& inputs,
    ...
    for (int n = 0; n < num_spatial_axes; ++n) {
      out_shape[n + 2] = deconv_out_size(input->shape(n + 2), kernel_size[n],
 ...
 private:
  ...
  std::shared_ptr<DeconvKernel> kernel_ = nullptr;
};
