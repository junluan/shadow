...
class OpRegistry {
 public:
  typedef Operator* (*Creator)(const shadow::OpParam&);
  ...
  static Operator* CreateOp(const shadow::OpParam& op_param) {
...
class OpRegister {
 public:
  OpRegister(const std::string& type,
             Operator* (*creator)(const shadow::OpParam&)) {
