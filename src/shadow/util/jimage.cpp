...
void JImage::Filter2D(const float *kernel, int height, int width) {
  ...
  unsigned char *data_f_index = data_f;
  for (int h = 0; h < h_; ++h) {
    for (int w = 0; w < w_; ++w) {
      ...
      for (int k_h = 0; k_h < height; ++k_h) {
        for (int k_w = 0; k_w < width; ++k_w) {
          ...
          val_c0 += data_[im_index + 0] * val_kernel;
          if (order_ != kGray) {
            val_c1 += data_[im_index + 1] * val_kernel;
            val_c2 += data_[im_index + 2] * val_kernel;
          }
        }
      }
      *data_f_index++ =
...
void JImage::GaussianBlur(int kernel_size, float sigma) {
...
void JImage::Canny(float thresh_low, float thresh_high, bool L2) {
  if (order_ != kGray) Color2Gray();

  int *grad_x = new int[h_ * w_], *grad_y = new int[h_ * w_];
  int *magnitude = new int[h_ * w_];
  Gradient(grad_x, grad_y, magnitude, L2);
...
#define CANNY_SHIFT 15
  ...
  memset(data_, 0, sizeof(unsigned char) * h_ * w_);
...
void JImage::GetGaussianKernel(float *kernel, int n, float sigma) {
...
void JImage::Gradient(int *grad_x, int *grad_y, int *magnitude, bool L2) {
  ...
  for (int h = 1; h < h_ - 1; ++h) {
    for (int w = 1; w < w_ - 1; ++w) {
      index = h * w_ + w;
      // grad_x[index] = data_[index + 1] - data_[index - 1];
      // grad_y[index] = data_[index + w_] - data_[index - w_];
      grad_x[index] = data_[index + w_ + 1] + data_[index - w_ + 1] +
                      (data_[index + 1] << 1) - data_[index + w_ - 1] -
                      data_[index - w_ - 1] - (data_[index - 1] << 1);
      grad_y[index] = data_[index - w_ - 1] + data_[index - w_ + 1] +
                      (data_[index - w_] << 1) - data_[index + w_ - 1] -
                      data_[index + w_ + 1] - (data_[index + w_] << 1);
      if (L2) {
