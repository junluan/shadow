...
void Yolo::Setup(int batch, VecRectF *rois) {
  ...
  im_ini_ = new JImage();
  im_res_ = new JImage(3, net_.in_shape()[2], net_.in_shape()[3]);
  if (rois == nullptr) {
...
void Yolo::Test(string image_file) {
  ...
  vector<VecBoxF> Bboxes;
  PredictYoloDetections(im_ini_, &Bboxes);
  Boxes::Amend(&Bboxes, rois_, im_ini_->h_, im_ini_->w_);
  VecBoxF boxes = Boxes::NMS(Bboxes, 0.5);
...
void Yolo::PredictYoloDetections(JImage *image, vector<VecBoxF> *Bboxes) {
  Bboxes->clear();
  ...
  for (int count = 0, b = 1; b <= batch_num; ++b) {
    ...
    for (int i = count; i < b * batch_ && i < num_im; ++i, ++c) {
      JImageProc::CropResize(*image, im_res_, rois_[i], net_.in_shape()[2],
                             net_.in_shape()[3]);
...
#if defined(USE_OpenCV)
void Yolo::DrawYoloDetections(const VecBoxF &boxes, cv::Mat *im_mat,
                              bool console_show) {
...
void Yolo::PrintYoloDetections(const VecBoxF &boxes, int count,
                               ofstream *file) {
  for (int b = 0; b < boxes.size(); ++b) {
    BoxF box = boxes[b];
